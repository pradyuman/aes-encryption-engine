...
COMPONENT_FILES	:= addRoundKey.sv AES_controller.sv encrypt.sv flex_counter.sv invSBox.sv invShiftRows.sv invSubBytes.sv key_schedule.sv mix_columns.sv sBox.sv shiftRows.sv subBytes.sv edge_detect.sv evolve_key_256.sv flex_stp_ring.sv evolve_key.sv add_alpha.sv xex_controller.sv tweak.sv data_route.sv xex_engine.sv AES_engine.sv xexaes256.sv scu.sv

# Specify the name of the top level file (do not include the source folder in the name)
# NOTE: YOU WILL NEED TO SET THIS VARIABLE'S VALUE WHEN WORKING WITH HEIRARCHICAL DESIGNS
# AND THE AUTOMATED GRADING SYSTEM
TOP_LEVEL_FILE	:= system.sv
...
# set_max_delay 5 -from "last_size[0]" -to "mask[127]"
# set_max_area 1000000
# set_max_total_power <power> mW
#$(if $(and $(CLOCK_NAME), $(CLOCK_PERIOD)), create_clock "$(CLOCK_NAME)" -name "$(CLOCK_NAME)" -period $(CLOCK_PERIOD))
# be safe, and use .5 ns faster
create_clock "clk" -name "clk" -period 4.6
# Step 3: Compile the design
#compile -map_effort medium
compile_ultra

# Step 4: Output reports
report_timing -path full -delay max -max_paths 3 -nworst 1 > reports/$(MOD_NAME).rep
